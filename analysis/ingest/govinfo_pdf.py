...
def _section_turns(
    ...
    for paragraph in paragraphs:
        ...
        if paragraph.kind == "boundary":
            ...
            continue
        sections = (
            non_spoken_sections(paragraph.text, submitted)
            if chamber == "senate" else [(paragraph.text, False)]
        )
        for text, is_submitted in sections:
            if submitted != is_submitted:
                yield from flush()
                attributed = False
                printing = is_submitted
            ...
            if submitted:
                current.append(text)
                continue
            marker = _SPEAKER_RE.match(text + " ")
            if paragraph.direct and marker and chamber in {"house", "senate"}:
                yield from flush()
                attributed = True
                printing = False
                floor_markers += 1
            elif paragraph.printed or marker or _ROLLCALL.match(text):
                yield from flush()
                attributed = False
                printing = True
            spoken, inserted = _split_inserted_material(text)
            if inserted:
                if spoken:
