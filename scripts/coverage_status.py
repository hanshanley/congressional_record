...
def _scan_manifest(path: Path, keep_ids: bool = False) -> Dict[str, Any]:
    ...
    When ``keep_ids`` is set the granuleId set is returned under ``_granule_ids``
    so callers can check that worker shards are fully merged into the main
    manifest. The key is stripped before the report is serialised.
    """
    if not path.exists():
    ...
    return {
        ...
        **({"_granule_ids": granules} if keep_ids else {}),
    }
